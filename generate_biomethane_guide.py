...
def build():
    ...
    story.append(make_table(
        ['事業領域', '概要', '規模感'],
        [
            ['天然ガス・LNG', 'LNG（液化天然ガス）の生産・輸送・販売。ブルネイ、マレーシア、オーストラリア、北米等で大型プロジェクトに参画', '取扱量 年間約3,000万トン\n日本のLNG輸入量の約15%に関与'],
            ['石油・石油化学', '原油トレーディング、精製事業。エネオスとの提携', '歴史ある事業だが縮小傾向'],
            ['再生可能エネルギー', '洋上風力（欧州・日本）、太陽光、蓄電池。Eneco（オランダ）を通じた欧州再エネ事業', 'Eneco：売上約6,000億円\n欧州最大級の再エネ事業者'],
            ['次世代エネルギー', '水素、アンモニア、CCUS（CO2回収・貯留）。エネルギートランジション領域', '開発段階の投資が中心'],
        ],
        [avail*0.16, avail*0.54, avail*0.30]
    ))
    ...
    story.append(info_box(
        ...
        '三菱商事は中期経営戦略で<b>「EX（エネルギートランスフォーメーション）」</b>を最重要テーマに掲げています。<br/><br/>'
        '具体的には：<br/>'
        '・化石燃料から再エネ・次世代エネルギーへの投資シフト<br/>'
        '・2030年までにポートフォリオの再エネ比率を大幅に引き上げ<br/>'
        '・欧州を先進的なエネルギー市場のテストベッドとして位置づけ<br/>'
        '・Enecoを軸にした欧州クリーンエネルギー事業の拡大'
    ))
    ...
    story.append(make_table(
        ['投資先', '国', '事業内容', '三菱商事の役割'],
        [
            ['Eneco', 'オランダ', '電力・ガス小売、洋上風力、太陽光', '100%子会社として経営'],
            ['洋上風力プロジェクト', 'オランダ・ベルギー・英国', '大型洋上風力発電所の開発・運営', 'コンソーシアムパートナー'],
            ['蓄電池事業', '英国・欧州各国', '大型蓄電池（BESS）の開発・運営', '事業投資'],
            ['水素・アンモニア', '欧州各国', 'グリーン水素サプライチェーン構築', '開発段階'],
        ],
        [avail*0.18, avail*0.15, avail*0.37, avail*0.30]
    ))

    story.append(Spacer(1, 3*mm))
    story.append(info_box(
        'ポイント：バイオメタンは「空白地帯」',
        '三菱商事は天然ガス・LNG・洋上風力・太陽光・蓄電池・水素と幅広くカバーしていますが、<br/>'
        '<b>バイオメタン/バイオガスへの本格参入はまだ行っていません。</b><br/>'
        'ここがカナデビアとの協業の「穴」であり、三菱商事にとっての新たな投資機会になり得ます。',
        WARN_BG, WARN_BORDER
    ))
    ...
    story.append(Paragraph('5.3 他の商社の動向', s_h2))
    story.append(Paragraph(
        '欧州バイオメタン領域への日本の商社の参入はまだ限定的です。早期参入のメリットがあります。', s_body))
    story.append(make_table(
        ['商社', '欧州バイオメタン関連の動き', '評価'],
        [
            ['三菱商事', 'Enecoを通じたガス事業基盤あり。バイオメタン特化の投資はまだ', '最も参入ポテンシャル高い'],
            ['三井物産', '欧州再エネ投資に積極的。バイオメタン特化は未確認', '潜在的な競合'],
            ['住友商事', '欧州の廃棄物発電事業に投資実績あり', '隣接領域での経験あり'],
            ['伊藤忠商事', '欧州再エネに一定の投資。バイオメタンは未参入', '参入可能性あり'],
            ['丸紅', '欧州のバイオマス発電に投資経験。ガス事業は限定的', 'バイオマス知見の転用可能'],
        ],
        [avail*0.14, avail*0.56, avail*0.30]
    ))
    ...
    story.append(make_table(
        ['手法', '指標', '一般的なレンジ', '使い方'],
        [
            ['DCF法', 'WACC 7～9%\nターミナル成長率 1.5～2.0%', '─', 'メインの評価手法。プラント別CFを積み上げ'],
            ['EV/EBITDA\nマルチプル', 'EV/EBITDA', '8～12x', 'ユーティリティ型は高め。EPC型は低め。\n直近の欧州バイオガスM&Aでは10x前後'],
            ['MW単価', 'EV/MW\n（発電等価容量）', '2～4M EUR/MW', 'プラントの規模をベースにした簡便法'],
            ['生産量単価', 'EV/Nm3\n（年間生産量）', '3～6 EUR/Nm3', 'バイオメタンの年間生産能力ベース'],
        ],
        [avail*0.14, avail*0.18, avail*0.20, avail*0.48]
    ))
    ...
    story.append(make_table(
        ['時期', '買い手', '売り手/対象', '概要'],
        [
            ['2024.12', 'KVI（カナデビア）', 'Iona Capital（英国）', '11か所のバイオガスプラント＋開発パイプラインを買収'],
            ['2024.3', 'KVI（カナデビア）', 'Schmack Biogas Srl（伊）', '北イタリアのバイオメタン事業者の過半数株式取得'],
            ['2024', 'TotalEnergies', '複数の欧州バイオガス事業者', '仏大手石油が欧州バイオメタン事業を積極買収'],
            ['2023', 'Shell', 'Nature Energy（デンマーク）', '約20億EURで欧州最大級のバイオメタン事業を買収。業界最大のM&A'],
            ['2023', 'bp', 'Archaea Energy（米国）', '約41億USDでバイオガス事業を買収（北米だが業界トレンドを示す）'],
        ],
        [avail*0.08, avail*0.18, avail*0.30, avail*0.44]
    ))
